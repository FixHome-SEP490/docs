...
Phiên bản: v1.4 \| Ngày: 14/09/2026 \| Trạng thái: CURRENT WORKING BASELINE
...
Nguồn nghiệp vụ: Master Project Specification v1.2 (13/09/2026) + quyết định nhóm ngày 14/09/2026 về FixHome-provided Parts, linh kiện ngoài do Technician đề xuất, optional paid part warranty và settlement. Những điểm mới được ghi rõ ở mục “Decision Delta”.
...
| Version       | v1.4                                                                                                                   |
| Ngày          | 14/09/2026                                                                                                             |
| Nguồn chính   | Master Project Specification v1.2 + quyết định nhóm ngày 14/09/2026 về linh kiện, optional paid warranty và settlement |
...
>
> 3\. Không khôi phục Deposit/Booking Hold/Wallet Reserve hoặc commission trên Parts nếu không có quyết định mới. FixHome-provided Parts chỉ tạo nghĩa vụ settlement theo giá trị linh kiện, không biến thành stored-value wallet.
>
...
> → PARTS nếu cần: FixHome Part hoặc linh kiện ngoài do Technician đề xuất  
> → Customer approve part source + price trước khi lắp; Technician Part mặc định NO_WARRANTY, Customer có thể chọn PAID_WARRANTY + warranty fee  
...
FixHome là intermediary marketplace/workflow platform và đồng thời có thể cung cấp một danh mục linh kiện dùng cho đơn sửa chữa. Current MVP chỉ quản lý linh kiện ở mức catalog cơ bản, source, selling price, warranty snapshot và xác nhận Technician đã nhận linh kiện; không xây Warehouse Management System, stock quantity, nhập/xuất/tồn, supplier procurement hoặc purchase order. FixHome không trực tiếp trở thành đơn vị thực hiện sửa chữa và không vận hành ví điện tử lưu tiền bảo đảm của Technician.
...
- Commission 10% trên Labor; Parts không commission. FixHome-provided Parts được tách riêng để platform thu đúng giá trị linh kiện; Technician-provided Parts thuộc khoản thu của Technician.
...
- Full Warehouse/Inventory/Supplier Procurement: stock quantity, nhập/xuất/tồn, barcode/bin location, purchase order, supplier replenishment, forecasting và warehouse workflow.
...
|-----------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Customer        | AI diagnosis; browse service/Tech; tạo Booking; shortlist; approve/reject Quote & Additional Cost; approve part source/price/warranty option; theo dõi order; payment; review; warranty claim.                                                          | Chọn Tech fail hard filter; sửa approved financial data; giả payment success; truy cập order người khác.                                                                                          |
| Technician      | Profile/KYC; skills/service area/schedule; listed labor price theo service; invitation; Accept/Decline; arrival; quote; select/pick up FixHome Part; đề xuất linh kiện mua ngoài khi cần; repair; evidence; cash confirm; PlatformDue; warranty rework. | Bypass verification; tự assign; sửa approved quote; tự đổi giá FixHome Part; lắp linh kiện mua ngoài khi Customer chưa approve; nhận job khi suspension/unpaid PlatformDue; giả provider success. |
| Service Manager | Exception/support: matching exhausted, dispute, arrival abnormal, cash mismatch/non-response, parts/warranty dispute, mid-job interruption, strike/priority review.                                                                                     | Thay quyền approve bình thường của Customer; forge payment; sửa approved quote không audit.                                                                                                       |
| Admin           | RBAC/users; KYC approval; service catalog; FixHome Part Catalog giá/warranty metadata; configs; ranking weights; geofence; strike policy; warranty options; commission rate; audit/reporting.                                                           | Retroactively mutate historical snapshots hoặc biến Part Catalog thành inventory/stock ledger trong current MVP.                                                                                  |
...
| 3      | Service & Parts Catalog              | Customer/Technician/Admin         | Service/category/reference pricing; TechnicianService listed labor price; lightweight FixHome Part Catalog (no stock management) |
...
| 8      | Quotation & Additional Cost          | Technician/Customer               | LABOR/PARTS, part source, price/warranty option approval, immutable revisions                                                    |
...
| 11     | Payments & PlatformDue               | Customer/Technician/Manager/Admin | Online/cash, labor commission, FixHome Parts settlement, ledger/audit                                                            |
...
| 14     | Warranty                             | Customer/Technician/Manager       | Coverage snapshot by labor/part source/provider, claim, rework, dispute                                                          |
...
| PlatformDue          | Khoản Technician phải thanh toán lại cho FixHome sau cash settlement = labor commission + giá trị FixHome-provided Parts đã invoice. Không phải wallet/reserve.                                      |
...
| Part Source          | Nguồn của PARTS_EQUIPMENT line item: FIXHOME hoặc TECHNICIAN; phải được snapshot khi Customer approve.                                                                                               |
| FixHome Part         | Linh kiện do FixHome cung cấp từ Part Catalog cơ bản; giá và warranty do FixHome quản lý. Technician có thể nhận/pick up nhưng không được tự sửa giá.                                                |
| Technician Part      | Linh kiện do Technician tự cung cấp khi FixHome Part không có/không phù hợp; Technician khai tên, giá, warranty và chịu trách nhiệm warranty đã cam kết.                                             |
| Part Warranty        | Coverage cho linh kiện cụ thể và tách khỏi Labor Warranty. FIXHOME Part có warranty theo catalog; TECHNICIAN Part mặc định không warranty, chỉ tạo coverage khi Customer mua optional paid warranty. |
...
| D-11   | FixHome được cung cấp linh kiện cho ServiceOrder, nhưng current MVP không triển khai Warehouse/Inventory Management.                                                                                                                                                                                               | FINALIZED                  | Bổ sung Part Catalog/source/warranty/settlement; không có stock ledger, PO hoặc supplier workflow.                                                                              |
| D-12   | Mọi PARTS_EQUIPMENT item phải có source = FIXHOME hoặc TECHNICIAN và Customer approve source + price + warranty trước khi lắp.                                                                                                                                                                                     | FINALIZED                  | Quotation/Additional Cost/Invoice cần snapshot source và warranty provider.                                                                                                     |
| D-13   | FixHome Part dùng giá + warranty do FixHome/Admin quản lý; Technician chỉ chọn item và xác nhận đã nhận/pick up, không được tự đổi giá.                                                                                                                                                                            | FINALIZED                  | Thêm lightweight PartCatalog + optional pickup confirmation; không track stock quantity.                                                                                        |
| D-14   | Nếu FixHome Part không có/không phù hợp, Assigned Technician được đề xuất linh kiện mua ngoài với giá cụ thể. Linh kiện này mặc định NO_WARRANTY; Customer có thể chọn PAID_WARRANTY với warrantyFee + warrantyTerm trước khi Approve.                                                                             | FINALIZED                  | External/Technician-recommended Part vẫn là approved financial item. Không có Part WarrantyCoverage nếu Customer không mua add-on; nếu mua thì Technician là warranty provider. |
| D-15   | Parts không chịu commission. Online/cash settlement phải tách FixHome Parts khỏi Technician entitlement; với cash dùng PlatformDue = labor commission + FixHome Parts total.                                                                                                                                       | RECOMMENDED IMPLEMENTATION | Giải quyết money flow mà không cần wallet/escrow hoặc inventory engine.                                                                                                         |
| D-16   | Part Warranty tách khỏi Labor Warranty. FIXHOME Part có coverage theo policy/catalog của FixHome. TECHNICIAN Part (linh kiện ngoài do thợ đề xuất) mặc định NO_WARRANTY; Customer có thể chọn PAID_WARRANTY và trả thêm warrantyFee, khi đó Technician là warranty provider và term/fee được snapshot khi approve. | FINALIZED                  | Không tạo Technician Part warranty nếu Customer không mua add-on; nếu mua thì claim route về Technician, Service Manager xử lý dispute/audit.                                   |

| Baseline v3.1 có PENDING_CONFIRMATION trong ServiceOrder. Quyết định D-01 của v1.2 và D-11→D-16 của v1.4 mới hơn và được dùng trong tài liệu này; khi cập nhật canonical Business Baseline tiếp theo phải sync lại rule/status/parts semantics tương ứng. |
|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
> → BEFORE Evidence  
> → Inspection  
> → Official Quotation (LABOR + PARTS + source + warranty option)  
> → Customer Approve?  
> No: no agreement → close/cancel job  
> Yes:  
> - nếu dùng FixHome Part: chọn approved Part Catalog item → Customer thấy fixed part price + included FixHome warranty → Technician pickup/receive confirmation  
> - nếu FixHome Part unavailable/unsuitable: Technician đề xuất linh kiện ngoài + price; mặc định NO_WARRANTY  
> - Customer có thể chọn PAID_WARRANTY cho Technician Part → cộng warrantyFee + snapshot warranty term/provider trước khi approve  
> → UNDER_REPAIR  
> → Additional Cost nếu phát sinh ngoài approved scope  
...
> Online: full invoice → provider → webhook verify → PAID → platform keeps labor commission + FixHome Parts value; Technician receives labor net + Technician Parts  
> Cash: Customer pays Tech → Tech declares amount → Customer confirms → CashSettlement confirmed → PlatformDue = labor commission + FixHome Parts value  
>   
> → ServiceOrder COMPLETED when completion/payment conditions satisfied  
> → Rating/Review + Repair History + FixHome Part WarrantyCoverage + optional paid WarrantyCoverage cho Technician Part
...
- Technician work suspension, account status và active unpaid PlatformDue phải được checked in eligibility.
...
- Fixed-price rules: (1) FixedBaseAmount = FixedUnitPriceSnapshot × Quantity; (2) Customer chấp nhận base price/scope khi Booking; (3) Technician không được tự đổi fixedPrice; (4) phát sinh ngoài fixed scope phải qua Additional Cost; (5) PARTS_EQUIPMENT luôn tách khỏi base service, phải có source FIXHOME/TECHNICIAN và Customer approval nếu phát sinh; (6) Parts không commission.
...
## 8.6 Technician Discovery & Booking Chat

- Trước khi tạo Booking, Customer được xem các Technician phù hợp/nearby gồm: **profile, rating, distance, Listed Labor Price và Typical Warranty**.

- Customer có thể xem và chọn các Technician ưu tiên khi tạo Booking.

- **Customer và Technician không được chat tự do trước khi Booking được tạo.**

- Sau khi Customer **Submit Booking**, hệ thống mở conversation gắn với Booking cho Customer và Technician được gửi Booking đó.

- Từ thời điểm này, **cả Customer và Technician đều có thể chủ động gửi tin nhắn** để trao đổi thêm về lỗi, hình ảnh, thời gian hoặc thông tin cần thiết.

- Chat phải gắn với BookingId; không tồn tại conversation độc lập ngoài Booking.

- Khi một Technician **Accept Booking**, conversation với Technician đó tiếp tục được sử dụng trong ServiceOrder.

- Các conversation với Technician khác của cùng Booking được đóng/read-only khi Booking đã có Technician được chọn.

- Chat không được dùng để thay đổi chính thức **price, quotation, schedule hoặc order status**; các thay đổi nghiệp vụ vẫn phải thực hiện qua chức năng tương ứng của hệ thống.

| **CHAT-BR-01:** Chat chỉ được kích hoạt sau khi Customer đã Submit Booking. Technician nhận Booking được phép trao đổi hai chiều với Customer. Không hỗ trợ chat tự do giữa Customer và Technician khi chưa tồn tại Booking. |
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
- Line items bắt buộc phân loại LABOR hoặc PARTS_EQUIPMENT; PARTS_EQUIPMENT phải có partSource = FIXHOME/TECHNICIAN. Với TECHNICIAN Part, warrantyOption mặc định = NO_WARRANTY; nếu Customer chọn PAID_WARRANTY thì phải snapshot warrantyFee + warrantyTerm.

- Customer thấy scope, labor, parts, part source, price và warranty option trước khi approve. FIXHOME Part hiển thị included warranty; TECHNICIAN Part hiển thị NO_WARRANTY hoặc PAID_WARRANTY + fee + term nếu Customer chọn.
...
> → LABOR/PARTS line items + part source + warranty option/fee nếu có  
...
FixHome được phép cung cấp linh kiện cho đơn sửa chữa, nhưng current MVP không quản lý kho theo nghĩa WMS/Inventory. Hệ thống chỉ cần biết linh kiện nào được phép dùng, giá bán, nguồn linh kiện, warranty snapshot và việc Technician đã nhận linh kiện khi cần.

· Mỗi PARTS_EQUIPMENT item phải snapshot partSource = FIXHOME hoặc TECHNICIAN; Customer approve trước khi lắp.

· FIXHOME: Technician chọn từ lightweight Part Catalog; sellingPrice và warranty do FixHome/Admin quản lý, Technician không được tự sửa. Có thể ghi pickedUpAt/receivedByTechnician để chứng minh đã nhận linh kiện, nhưng không track stock quantity.

· TECHNICIAN: dùng cho linh kiện mua ngoài do Assigned Technician đề xuất khi FixHome Part không có/không phù hợp hoặc Customer chọn phương án đó. Technician khai tên/model nếu có, giá và notes; Customer Approve/Reject trước khi sử dụng. Mặc định linh kiện này NO_WARRANTY.

· Optional paid warranty: Customer có thể chọn PAID_WARRANTY cho TECHNICIAN Part trước khi approve/lắp. Hệ thống lưu warrantyFee, warrantyTerm và provider=TECHNICIAN; warrantyFee được cộng vào invoice. Nếu Customer không chọn thì hệ thống không tạo Part WarrantyCoverage cho linh kiện đó.

· Nếu FixHome Part đã được nhận nhưng cuối cùng không sử dụng, item đó không được invoice. Việc hoàn trả vật lý về kho/điểm nhận là operational process ngoài software scope.

· Parts không commission. FIXHOME Part có warranty theo FixHome policy. TECHNICIAN Part mặc định không bảo hành; Technician chỉ trở thành warranty provider nếu Customer đã chọn và thanh toán PAID_WARRANTY. Service Manager chỉ xử lý exception/dispute và audit.
...
PlatformDue sau cash là Technician obligation riêng và không giữ Customer ServiceOrder mở sau khi Customer-side completion/payment đã được resolved.

## 8.16 Pricing & Commission

> INSPECTION_REQUIRED:  
> FinalLaborTotal = BaseLabor + ApprovedAdditionalLabor  
> FinalFixHomePartsTotal = Approved FixHome Parts  
> FinalTechnicianPartsTotal = Approved Technician Parts  
> FinalTechnicianPartWarrantyFee = Approved paid warranty fees for Technician Parts  
>   
> FIXED_PRICE:  
...
> FinalTechnicianPartWarrantyFee = Approved paid warranty fees for Technician Parts  
>   
> FinalPartsTotal = FinalFixHomePartsTotal + FinalTechnicianPartsTotal  
> FinalInvoiceAmount = FinalLaborTotal + FinalPartsTotal + FinalTechnicianPartWarrantyFee  
> PlatformCommission = FinalLaborTotal × CommissionRateSnapshot  
> PlatformReceivable = PlatformCommission + FinalFixHomePartsTotal  
> TechnicianNetEntitlement = FinalLaborTotal + FinalTechnicianPartsTotal + FinalTechnicianPartWarrantyFee - PlatformCommission  
> Current CommissionRate = 10%
>
- Parts không commission. FIXHOME Parts là platform receivable theo selling-price snapshot; TECHNICIAN Parts thuộc Technician entitlement sau Customer approval. Optional Technician Part warrantyFee cũng được ghi riêng trên invoice; current MVP không đưa khoản này vào labor commission base.

- Commission rate phải snapshot để config tương lai không làm đổi lịch sử.

- Mọi total phải trace được về approved quotation/additional items, part source và immutable snapshots.
...
> → platform retains PlatformCommission + FinalFixHomePartsTotal  
> → Technician settlement = TechnicianNetEntitlement
>
...
## 8.18 Cash Payment / Cash Confirmation / PlatformDue
...
> → create PlatformDue = PlatformCommission + FinalFixHomePartsTotal
>
...
- Active unpaid PlatformDue block Technician nhận/Accept tất cả new booking; không block login/history/payment access. Nếu order không có FixHome Part thì PlatformDue chính là labor commission như logic CommissionDue cũ.
...
| Proposed Warranty | Tech khai Labor Warranty nếu có. Với TECHNICIAN Part, mặc định NO_WARRANTY; chỉ khi Customer chọn PAID_WARRANTY thì Tech khai warrantyFee/term trước approval. FIXHOME Part dùng policy từ Part Catalog.                                                                                                                     |
| WarrantyCoverage  | Khi approve/install: LABOR và FIXHOME Part snapshot coverage bình thường. TECHNICIAN Part chỉ tạo PART coverage nếu warrantyOption=PAID_WARRANTY và fee đã được approve; provider=TECHNICIAN, term/days/item được snapshot.                                                                                                  |
| Warranty Claim    | Customer chỉ claim khi tồn tại WarrantyCoverage. Technician xử lý Labor Warranty và paid warranty của TECHNICIAN Part; FixHome/Service Manager điều phối FIXHOME Part; dispute mới escalate theo audit.                                                                                                                      |
| Part Warranty     | Bảo hành linh kiện tách khỏi bảo hành công. FIXHOME Part có warranty theo FixHome policy. TECHNICIAN Part mặc định NO_WARRANTY; chỉ khi Customer mua PAID_WARRANTY thì Technician chịu coverage theo term đã approve. Part warranty không tự động bao gồm labor nếu Labor Warranty đã hết, trừ khi approved coverage ghi rõ. |

Warranty chỉ cover lỗi/scope/item cũ thuộc coverage đã snapshot. Với FIXHOME Part bị xác nhận lỗi trong thời hạn, Customer không trả lại giá linh kiện thay thế; việc labor có miễn phí hay không phụ thuộc Labor Warranty/approved coverage của job. Với TECHNICIAN Part, mặc định không có warranty; chỉ khi Customer đã mua PAID_WARRANTY thì Technician chịu warranty đúng term đã snapshot. Lỗi mới không liên quan lần sửa trước phải tạo Additional Quotation riêng; Customer Approve thì mới sửa và chịu khoản mới.
...
Service Manager là exception/support operator, bao gồm parts/warranty dispute và cash/platform due exception, không phải dispatcher bình thường. Admin quản config, verification, user/RBAC, service catalog, lightweight FixHome Part Catalog (name/price/warranty/active), ranking/warranty/commission policy và audit; không quản stock movement trong current MVP.
...
| FixHome Part unavailable/unsuitable     | Assigned Tech may propose TECHNICIAN Part with explicit price; default NO_WARRANTY, optional PAID_WARRANTY + fee | Only dispute               | Customer must Approve part price and any paid warranty add-on; no forced substitution.                                                    |
| FixHome Part picked up but not used     | Exclude from invoice; mark not-used/void item                                                                    | Only abnormal/dispute      | Physical return handled offline; no inventory workflow.                                                                                   |
| FixHome Part warranty claim             | Validate WarrantyCoverage/provider → coordinate replacement/rework                                               | Yes if needed              | Part replacement value covered by FixHome; labor follows separate Labor Warranty.                                                         |
| Paid Technician Part warranty dispute   | Review approved item, paid warranty selection/fee/term, evidence/timeline                                        | Yes                        | Only applies when PAID_WARRANTY was purchased; Technician is warranty provider only for purchased PAID_WARRANTY; Manager mediates/audits. |
...
| PlatformDueStatus    | PENDING, PAID, FAILED                                           |
...
| Pay PlatformDue                                    | No             | Due owner Tech       | Support             | Audit                                     |
...
| View part source/price/warranty option             | Order owner    | Assigned Tech        | Exception scope     | Audit/config                              |
| Select FixHome Part for order                      | No             | Assigned Tech        | Support only        | Catalog config only                       |
| Confirm FixHome Part pickup                        | View own order | Assigned Tech        | Review              | Audit                                     |
| Propose Technician Part                            | No             | Assigned Tech        | No normal           | No                                        |
| Approve Part source/price + optional paid warranty | Order owner    | No                   | No normal           | No                                        |
| Manage FixHome Part Catalog                        | No             | No                   | No                  | Yes: name/price/warranty/active; no stock |
...
| Quotation / QuotationItem              | Versioned quote + LABOR/PARTS items + partSource + PartCatalog/custom snapshot + warrantyOption; TECHNICIAN Part may snapshot warrantyFee/term when paid warranty selected |
| AdditionalCostRequest / Item           | Versioned approved delta; PARTS item keeps source, price and warranty-option snapshots                                                                                     |
| CustomerServiceConfirmation            | Completion decision/timestamp                                                                                                                                              |
| Invoice / InvoiceItem                  | Final immutable financial snapshot; each Parts item retains FIXHOME/TECHNICIAN source                                                                                      |
...
| PlatformDue                            | Cash obligation from Technician to platform = labor commission + invoiced FixHome Parts value                                                                              |
...
| WarrantyCoverage                       | Approved warranty snapshot per covered labor/part item; TECHNICIAN Part coverage exists only when paid warranty was selected                                               |
...
| PartCatalog                            | Lightweight FixHome parts list: id/SKU optional, name, description, sellingPrice, warrantyDays/policy, active. Explicitly no stock quantity/procurement fields in MVP.     |
| PartPickupConfirmation                 | Optional audit record/timestamp proving Assigned Technician received an approved FIXHOME Part; not an inventory movement ledger.                                           |
...
> PartCatalog 1 ── \* approved Parts line items (optional reference only when partSource=FIXHOME)  
> Approved FIXHOME Part item 1 ── 0..1 PartPickupConfirmation
...
- Indexes cho Booking customer/status/time; Invitation technician/status/expiry; ServiceOrder status; PlatformDue technician/status; TechnicianService service/active; PartCatalog active/name; geospatial strategy tùy implementation.
...
- PARTS_EQUIPMENT validation: partSource required; FIXHOME requires PartCatalog snapshot/ref and locked platform price; TECHNICIAN requires custom part name/price + warrantyOption snapshot; optional warrantyFee/warrantyTerm khi Customer chọn PAID_WARRANTY. Không thêm stockQuantity vào current MVP.
...
| POST /platform-dues/:id/pay                                | Due owner Tech            | Pay labor commission + FixHome Parts amount due to platform           |
...
| GET /parts/catalog                                         | Assigned Technician/Admin | Browse lightweight active FixHome Parts list; no stock quantity       |
| POST /service-orders/:id/parts/:itemId/pickup-confirmation | Assigned Technician       | Confirm receipt/pickup of approved FIXHOME Part                       |
| POST /admin/parts, PATCH /admin/parts/:id                  | Admin                     | Manage FixHome Part name/price/warranty/active; no inventory movement |
...
| Database            | Supabase PostgreSQL                              | Structured business data, constraints, ledger/history, lightweight Part Catalog and immutable source/warranty snapshots; no WMS/stock ledger |
...
| Payment             | Provider/Bank integration                        | Online invoice payment, PlatformDue, settlement/payout as provider supports                                                                  |
...
| PlatformDue gate        | Eligibility query must exclude Tech with active unpaid PlatformDue.                                                                                                                                           |
...
| Parts source & warranty | Reject PARTS item without source/warrantyOption snapshot; FIXHOME price must come from active PartCatalog snapshot; TECHNICIAN Part defaults NO_WARRANTY unless paid warranty fields are explicitly approved. |
| Parts settlement        | Online/cash settlement must separate FIXHOME Parts from TECHNICIAN Parts and never calculate commission on Parts.                                                                                             |
...
- Labor/Parts từng khoản, Parts source/provider và final invoice?

- Commission rate snapshot?

- Online/cash payment confirmed bằng gì?

- PlatformDue (commission + FixHome Parts nếu có) paid chưa?
...
|-----------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Customer        | Home/Service Search → AI Diagnosis → Technician List/Profile + Listed Price → optional contact → Create Booking → shortlist/order → matching → active order → quote/part source + optional warranty selection → additional approval → payment → review/history/warranty.                                                                            |
| Technician      | Onboarding/KYC → Profile/Services/Listed Price → Schedule/Area → Invitation Inbox → Accept/Decline → En Route/Check-in → Evidence → Quote → select FIXHOME Part or propose external Part → optional paid warranty selection → pickup confirmation if needed → Repair/Additional Cost → Completion/Cash confirmation → PlatformDue → Warranty cases. |
| Service Manager | Exception Queue → Case Detail/Timeline/Evidence → payment/parts/cancellation/warranty/reassignment resolution → audit outcome.                                                                                                                                                                                                                      |
| Admin           | Dashboard → Users/RBAC → KYC Verification → Service Catalog → FixHome Part Catalog (price/warranty/active, no stock) → Configs → payment/platform due audit → system audit/reporting.                                                                                                                                                               |
...
|-------------------------|--------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------|
| P0 – Foundation         | Auth/JWT/RBAC; User; Service Catalog; lightweight FixHome Part Catalog; Technician profile/KYC; common validation/config/API conventions   | Nền cho mọi module.                           |
...
| P2 – Order Execution    | ServiceOrder state machine; maps/check-in; evidence; quotation; part source selection/pickup confirmation; additional cost; completion     | Core repair workflow.                         |
| P3 – Finance            | Invoice; online payment integration; cash dual confirmation; PlatformDue; settlement gates                                                 | Khóa revenue/settlement semantics.            |
| P4 – Quality/Exceptions | Cancellation/strike/boost; Service Manager support; labor + FixHome part warranty + optional paid Technician-part warranty; review/history | Xử lý thực tế và audit.                       |
...
|------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Core/Leader            | Auth, JWT, RBAC, users, KYC verification plumbing, Service Catalog, FixHome Part Catalog metadata, shared config/validation, integration/code review                                                                                                                                        |
| Booking/Order          | Booking, ServiceOrder, state machine, quotation, part source/warranty-option snapshots, pickup confirmation, additional cost, cancellation, evidence, completion                                                                                                                            |
...
| Happy path fixed-price | Customer books fixed service × quantity → price snapshot → Tech Accept/arrival → base scope without quote → optional approved Part (FIXHOME/TECHNICIAN) + optional paid Technician-part warranty + Additional Cost → invoice → commission 10% labor only; warranty follows approved option. |
...
- Commission/PlatformReceivable/TechnicianNetEntitlement formulas

- Strike/suspension evaluator

- Labor/part WarrantyCoverage routing by provider/source

- DTO/business validations

- PARTS source validation + FixHome Part price lock
...
- PlatformDue eligibility gate

- Storage metadata ownership

- Cash/online settlement with mixed FIXHOME + TECHNICIAN Parts
...
| Happy path cash                        | Cash dual confirm→PlatformDue created (commission + FixHome Parts if any)→order Completed→Tech blocked new jobs until due paid                                          |
...
| FixHome Part happy path                | Quote/Additional Cost uses FIXHOME source→Customer approves price + included warranty→Tech pickup confirms→repair→invoice keeps source→FixHome part warranty snapshot   |
| FixHome Part unavailable               | Tech proposes TECHNICIAN Part with price→default NO_WARRANTY; Customer may add PAID_WARRANTY + fee→approve→repair                                                       |
| Part warranty claim                    | Claim only exists when WarrantyCoverage exists: FIXHOME Part covered by FixHome; TECHNICIAN Part only when paid warranty was purchased; Labor Warranty remains separate |
...
| BRX-030 | Cash PlatformDue contains labor commission and, when applicable, invoiced FixHome Parts value.                                                                                                                                                                                 |
| BRX-031 | Active unpaid PlatformDue blocks all new work acceptance, not login/payment access.                                                                                                                                                                                            |
...
| BRX-051 | FixHome may provide parts for approved repairs, but current MVP does not implement warehouse/inventory/procurement management.                                                                                                                                                 |
| BRX-052 | Every PARTS_EQUIPMENT line item must snapshot partSource = FIXHOME or TECHNICIAN before Customer approval.                                                                                                                                                                     |
| BRX-053 | FIXHOME Part price and warranty are controlled by FixHome/Admin Part Catalog and snapshotted on approval; Technician cannot edit that selling price.                                                                                                                           |
| BRX-054 | Assigned Technician may propose a TECHNICIAN Part when FixHome Part is unavailable/unsuitable; name, price and notes must be explicit and Customer-approved before installation. Default warrantyOption = NO_WARRANTY.                                                         |
| BRX-055 | FIXHOME Part has Part Warranty according to FixHome policy. TECHNICIAN Part has no default warranty; only when Customer buys PAID_WARRANTY does Technician become warranty provider. Part Warranty and Labor Warranty are separate coverages.                                  |
| BRX-056 | Parts are never commissionable. Online settlement retains FixHome Parts value + labor commission for platform and pays Technician labor net + Technician Parts value.                                                                                                          |
| BRX-057 | For cash, PlatformDue = PlatformCommission + FinalFixHomePartsTotal; active unpaid PlatformDue blocks Technician from accepting new work.                                                                                                                                      |
| BRX-058 | A FIXHOME Part that was picked up but not used is excluded from invoice; physical return is handled offline and does not require stock movement features in MVP.                                                                                                               |
| BRX-059 | Current Part Catalog stores descriptive/price/warranty/active metadata only; no stockQuantity, warehouse location, purchase order, supplier replenishment or inventory ledger is required.                                                                                     |
| BRX-060 | For TECHNICIAN Part, Customer may optionally select PAID_WARRANTY before approval/installation. The warrantyFee and warrantyTerm must be explicit and snapshotted; if not selected, no Part WarrantyCoverage is created.                                                       |
| BRX-061 | RECOMMENDED IMPLEMENTATION: Record Technician Part warrantyFee separately on the invoice and keep it outside FinalLaborTotal so the existing 10% labor commission rule is unchanged. If the team wants commission on warrantyFee, it requires a new explicit finance decision. |
...
- FixHome Part selling price + warranty term/policy (catalog metadata only; not stock quantity)
...
| TBD-FIN-01      | Customer cancels after approved TECHNICIAN Part was actually purchased specifically for the job: reimbursement/ownership rule? | Manager exception/manual evidence resolution for MVP; do not invent automatic charge. FIXHOME Part picked up but unused is excluded from invoice and returned offline. |
...
| TBD-FIN-02      | Optional warrantyFee của TECHNICIAN Part có chịu platform commission hay không?                                                | Keep warrantyFee as a separate invoice component and outside FinalLaborTotal for MVP until the team explicitly confirms a commission policy.                           |
...
- Full Warehouse/Inventory/Supplier Procurement module (stock level, nhập/xuất/tồn, purchase order, supplier, barcode)
...
> 8) Official Quote: Labor 300k + FixHome Part “Board X” 700k, Part Warranty 90 days, Labor Warranty 30 days.  
> 9) Customer Approve → A confirms pickup/receipt of FixHome Part → UNDER_REPAIR.  
> 10) Discover extra labor 100k → Additional Cost → Customer Approve.  
> 11) FinalLaborTotal=400k; FinalFixHomePartsTotal=700k; FinalTechnicianPartsTotal=0; Invoice=1.1m; Commission=40k.  
> 12a) Online: Customer pays 1.1m; platform retains 740k (700k FixHome Part + 40k commission); TechnicianNetEntitlement=360k.  
> 12b) Cash: Customer pays A 1.1m; A confirms; Customer confirms; PlatformDue=740k; A economic net after due=360k.  
> 13) AFTER evidence + confirmation/payment conditions → COMPLETED.  
> 14) Nếu Board X lỗi trong 90 ngày: claim theo FIXHOME Part warranty. Labor chỉ miễn phí nếu còn Labor Warranty hoặc coverage ghi rõ.  
> 15) Nếu FixHome không có Board phù hợp: A có thể đề xuất linh kiện mua ngoài với giá cụ thể. Mặc định NO_WARRANTY. Nếu Customer muốn bảo hành thì chọn PAID_WARRANTY, hệ thống cộng warrantyFee và snapshot warrantyTerm; chỉ khi đó A là warranty provider.
...
- ☐ Parts never commissionable; every Parts item has source FIXHOME/TECHNICIAN and immutable price + warranty-option snapshot.

- ☐ Online pays full invoice and is server-verified.

- ☐ Cash dual confirmation; no auto-confirm; PlatformDue = labor commission + FixHome Parts and blocks new work until paid.
...
- ☐ Labor Warranty and Part Warranty are separate; FIXHOME Part provider = FixHome; TECHNICIAN Part mặc định NO_WARRANTY và chỉ có provider=Technician khi Customer đã mua PAID_WARRANTY; new unrelated issue needs new quotation.
...
- ☐ Commission remains 10% of commissionable labor/service value; Parts are excluded. FixHome Parts are platform receivable, not commission.

- ☐ No Warehouse/Inventory Management in current MVP: no stock quantity, nhập/xuất/tồn, PO or supplier workflow.

- ☐ FixHome Part unused → excluded from invoice; physical return handled offline.
...
FixHome hiện có core business tương đối ổn định và có thể triển khai theo kiến trúc modular/layered. Service Catalog hỗ trợ FIXED_PRICE và INSPECTION_REQUIRED; Customer tạo Booking cho một Primary Service; backend hard-filter/rank Technician; Customer shortlist tối đa 5; invitations tuần tự; Technician Accept mới tạo ServiceOrder. Với linh kiện, FixHome có thể cung cấp lightweight Part Catalog nhưng không xây inventory/WMS; mỗi Parts item phải xác định source FIXHOME hoặc TECHNICIAN, giá và warranty được Customer approve/snapshot. FixHome Parts do FixHome bảo hành theo coverage, Technician Parts do Technician bảo hành; labor warranty tách riêng. FixHome lấy commission 10% trên labor/service value, không commission Parts; online/cash settlement tách đúng giá trị FixHome Parts và Technician entitlement.

Các member có thể dùng tài liệu này làm guide chung để chia module, thiết kế ERD/API/UI, viết test và demo. Phạm vi mới về Parts dừng ở catalog/source/approval/pickup confirmation/warranty/settlement; mọi chức năng inventory, stock movement, supplier procurement được để cho future development. Những điểm được đánh dấu TBD phải được giữ nguyên là TBD cho tới khi nhóm ra quyết định mới.
